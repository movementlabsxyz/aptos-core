module aptos_framework::native_bridge {
    ...
    public(friend) fun is_inbound_nonce_set(
        bridge_transfer_id: vector<u8>
    ): bool acquires SmartTableWrapper {
    ...
    public(friend) fun create_details(
        ...
        OutboundTransfer { bridge_transfer_id, initiator, recipient, amount }
    }
    ...
    public(friend) fun set_bridge_transfer_id_to_inbound_nonce(
        bridge_transfer_id: vector<u8>, inbound_nonce: u64
    ...
    public entry fun initiate_bridge_transfer(
        initiator: &signer, recipient: vector<u8>, amount: u64
    ...
    public fun update_bridge_fee(relayer: &signer, new_bridge_fee: u64) acquires BridgeConfig {
    ...
    fun test_update_bridge_relayer(
        ...
        assert!(
            event::was_event_emitted<BridgeConfigRelayerUpdated>(
                &BridgeConfigRelayerUpdated { old_relayer: @aptos_framework, new_relayer }
            ),
            0
        );
    ...
    #[test(aptos_framework = @aptos_framework)]
    /// Tests updating the bridge relayer and emitting the corresponding event.
    fun test_update_bridge_fee(aptos_framework: &signer) acquires BridgeConfig {
        let new_fee = 100;
        initialize_for_test(aptos_framework);
    ...
    fun test_failing_update_bridge_relayer(
        aptos_framework: &signer, bad: &signer, new_relayer: address
    ) acquires BridgeConfig {
        initialize_for_test(aptos_framework);
    ...
    #[
        test(
            ...
            relayer = @0xcafe,
            sender = @0x726563697069656e740000000000000000000000000000000000000000000000
        )
    ]
    fun test_initiate_bridge_transfer_happy_path(
        sender: &signer, aptos_framework: &signer, relayer: &signer
    ...
    fun test_complete_bridge_transfer_by_non_relayer(
        sender: &signer, aptos_framework: &signer
    ) acquires BridgeEvents, AptosCoinMintCapability, SmartTableWrapper, BridgeConfig {
        let sender_address = signer::address_of(sender);
        // Create an account for our recipient
        initialize_for_test(aptos_framework);
    ...
    #[test(aptos_framework = @aptos_framework, sender = @0xdaff)]
    #[expected_failure(abort_code = EINVALID_BRIDGE_TRANSFER_ID, location = Self)]
    // ENOT_FOUND
