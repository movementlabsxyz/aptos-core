module aptos_framework::governed_gas_pool {
    ...
    public fun initialize(
        aptos_framework: &signer, delegation_pool_creation_seed: vector<u8>
    ) {
        system_addresses::assert_aptos_framework(aptos_framework);

        // return if the governed gas pool has already been initialized
        if (exists<GovernedGasPool>(signer::address_of(aptos_framework))) { return };
        ...
        let (governed_gas_pool_signer, governed_gas_pool_signer_cap) =
            account::create_resource_account(aptos_framework, seed);
        ...
        move_to(
            aptos_framework,
            GovernedGasPool { signer_capability: governed_gas_pool_signer_cap }
        );
    }
    ...
    fun governed_gas_signer(): signer acquires GovernedGasPool {
        let signer_cap =
            &borrow_global<GovernedGasPool>(@aptos_framework).signer_capability;
    ...
    public fun fund<CoinType>(
        aptos_framework: &signer, account: address, amount: u64
    ...
    fun deposit_from_fungible_store(account: address, amount: u64) acquires GovernedGasPool {
        if (amount > 0) {
            ...
            let governed_gas_pool_address = governed_gas_pool_address();
            let governed_gas_pool_store_address =
                primary_fungible_store_address(governed_gas_pool_address);
            ...
            fungible_asset::deposit_internal(
                ...
                fungible_asset::withdraw_internal(account_store_address, amount)
            );
        }
    }
    ...
    public fun store_aptos_coin_mint_cap(
    ...
    public fun store_aptos_coin_burn_cap(
    ...
    fun test_governed_gas_pool_deposit(
        aptos_framework: &signer, depositor: &signer
    ) acquires GovernedGasPool, AptosCoinMintCapability {
        ...
        let governed_gas_pool_balance =
            coin::balance<AptosCoin>(governed_gas_pool_address());
        ...
        assert!(
            coin::balance<AptosCoin>(signer::address_of(depositor))
                == depositor_balance - 100,
            1
        );
        assert!(
            coin::balance<AptosCoin>(governed_gas_pool_address())
                == governed_gas_pool_balance + 100,
            2
        );
    ...
    fun test_governed_gas_pool_deposit_gas_fee(
        aptos_framework: &signer, depositor: &signer
    ) acquires GovernedGasPool, AptosCoinMintCapability {
    ...
    fun test_governed_gas_pool_get_balance(
    ...
    fun test_governed_gas_pool_fund(
        aptos_framework: &signer, depositor: &signer, beneficiary: &signer
    ) acquires GovernedGasPool, AptosCoinMintCapability {
        ...
        assert!(
            coin::balance<AptosCoin>(signer::address_of(beneficiary)) == 100,
            4
        );
