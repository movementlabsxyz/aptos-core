...
module aptos_framework::stake {
    ...
    /// power.
    public(friend) fun on_new_epoch() acquires StakePool, ValidatorConfig, ValidatorPerformance, ValidatorSet, ValidatorFees {
        let validator_set = borrow_global_mut<ValidatorSet>(@aptos_framework);
        let config = staking_config::get();
        let validator_perf = borrow_global_mut<ValidatorPerformance>(@aptos_framework);
    ...
    public fun initialize_test_validator(
        ...
        should_end_epoch: bool
    ) acquires AptosCoinCapabilities, AllowedValidators, OwnerCapability, StakePool, ValidatorConfig, ValidatorPerformance, ValidatorSet, ValidatorFees {
    ...
    fun test_validator_consensus_infos_from_validator_set(
        ...
        assert!(
            vector[@0x5, @aptos_framework, @0x3] == vci_addrs,
            1
        );
        assert!(
            vector[pk_5_bytes, pk_1_bytes, pk_3_bytes] == vci_pks,
            2
        );
    ...
    #[test_only]
    public fun end_epoch() acquires StakePool, ValidatorConfig, ValidatorPerformance, ValidatorSet, ValidatorFees {
    ...
    fun test_distribute_validator_fees(
        ...
        // Fees have to be assigned to the right validators, but not
        // distributed yet.
