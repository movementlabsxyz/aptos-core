...
impl<'a> BinaryModuleLoader<'a> {
    ...
    fn add_or_update_fun(
        ...
        let acquired_structs = def_view
            .as_ref()
            .map(|(_, def_view)| self.acquired_structs(def_view));
        ...
        if new || has_def {
            ...
            fun_data.acquired_structs = acquired_structs;
    ...
    fn acquired_structs(
        ...
        let module = def_view.module();
        def_view
            .acquired_resources()
            .iter()
            .map(|struct_def_idx| {
                let struct_handle_idx = module.struct_def_at(*struct_def_idx).struct_handle;
                let struct_handle = module.struct_handle_at(struct_handle_idx);
                let name = module.identifier_at(struct_handle.name);
                StructId::new(self.sym(name.as_str()))
            })
            .collect()
    }
